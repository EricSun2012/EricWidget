...
public class RoundProgressBar extends View {
    ...
    public RoundProgressBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        max = mTypedArray.getInteger(R.styleable.RoundProgressBar_max, 1);
        textIsDisplayable = mTypedArray.getBoolean(
                R.styleable.RoundProgressBar_textIsDisplayable, true);
        style = mTypedArray.getInt(R.styleable.RoundProgressBar_style, 0);
        progress = mTypedArray.getFloat(R.styleable.RoundProgressBar_progressSize, 0);
        mTypedArray.recycle();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int percent = (int) ((progress / max) * 100); // 中间的进度百分比，先转换成float在进行除法运算，不然都为0
        float textWidth = paint.measureText(percent + "%"); // 测量字体宽度，我们需要根据字体的宽度设置在圆环中间

        if (textIsDisplayable && style == STROKE) {
            canvas.drawText(percent + "%", centre - textWidth / 2, centre - 2
        ...
        switch (style) {
            case STROKE: {
                ...
                canvas.drawArc(oval, 270, 360 * progress / max,
                        false, paint); // 根据进度画圆弧
            }
            break;
            case FILL: {
                ...
                if (progress != 0)
                    canvas.drawArc(oval, 270, 360 * progress
                            / max, true, paint); // 根据进度画圆弧
            }
            break;
        }
